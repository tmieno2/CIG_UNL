...
#- For individual ffy
#---------------------
make_grower_folder_files <- function(ffy) {
  ...
  #++++++++++++++++++++++++++++++++++++
  add_info_rmd <-
    readLines(here::here("DataProcessing/add_info_to_json_template.rmd")) %>%
    gsub("ffy_here", ffy, .)
  ...
  writeLines(
    ...
    paste0(
      root_dir,
      "/DataProcessingReport/add_info_to_json.rmd"
    )
  )

  #++++++++++++++++++++++++++++++++++++
  #+ create process_data.rmd
  #++++++++++++++++++++++++++++++++++++
  process_data_rmd <-
    readLines(here::here("DataProcessing/process_data_template.rmd")) %>%
    gsub("ffy_here", ffy, .)
  ...
  writeLines(
    ...
    paste0(
      root_dir,
      "/DataProcessingReport/process_data.rmd"
    )
  )
}

#---------------------
#- For all the entries in the json file
#---------------------
make_grower_folders <- function() {
...
gen_fp_template <- function(farm, field, year, crop, input_ls, strategy_ls, json_file = NULL) {
  temp_data <- data.table(
    ...
    rotation = "CS or CC or SW",
    prev_crop = "S or C or W",
    plater_rows = "numeric (no double quotes needed)",
    row_spacing = "numeric (no double quotes needed)",
    harvester_rows = "numeric (no double quotes needed)",
    tillage = "yes or no",
    till_dates = list("dates xx/yy/yy sep by comma"),
    planting_date = "date",
    planting_depth = "in",
    hybrid = "one word",
    population = "ac",
    irrigation_type = "pivot, linear or other",
    irrigation_total = "in",
    seed_trt = "name",
    herbicides = "name",
    foliar = "name",
    foliar_fungicide = "name"
  )
...
initiate_fp_entry <- function(farm, field, year, crop) {
  temp_data <-
    data.table(
      trial_supervisor = "supervisor name",
      researcher = "institution name (e.g., UIUC, UMT, LSU)",
  ...
  if (temp_data$ffy %in% existing_data$ffy) {
    print("An experiment with the same farm, field, year combination already exisists in the metadata.json. ")
  } else {
    temp_data <- rbind(existing_data, temp_data, fill = TRUE)

    #--- update the meatadata.json with the new entry ---#
    jsonlite::write_json(
      temp_data,
      file.path(
        here("Data", "CommonData"),
    ...
    #--- create folders and R template programs ---#
    make_grower_folder_files(ffy = paste(farm, field, year, sep = "_"))
  }
}
...
add_basic_info <- function(ffy, basic_data) {
  ...
  w_data$yield_data <- basic_data$yield_file
  w_data$tr_data <- basic_data$trial_design_file
  w_data$h_width <- basic_data$harvester_width
