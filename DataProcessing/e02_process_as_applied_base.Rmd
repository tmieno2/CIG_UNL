...
## If the input data is in polygon already, then save and exit

```{r}
if (is_aa_input_polygon == TRUE) {
  aa_input_sf <-
    aa_input %>%
    flag_bad_points(., "input_rate", sd_factor) %>%
    mutate(aa_input_id = 1:n()) %>%
...
if (w_Rx_data != "none") { # if Rx data exists
  ...
  Rx_data <-
    ...
    filter(input_id == w_input_id) %>%
    pull(Rx_data) %>%
    here::here("Data/Growers", ffy, "Raw", .) %>%
    paste0(., ".shp") %>%
    st_read() %>%
...
if (length(zero_rx_id) != 0) {
  ...
  group_cluster <-
    ...
    nest_by(rx_id) %>%
    mutate(data = list(
      if (nrow(data) > num_paths_per_Rx_plot) {
        mutate(data, cluster = kmeans(w_group, num_paths_per_Rx_plot)$cluster)
      } else if (nrow(data) == num_paths_per_Rx_plot) {
        mutate(data, cluster = w_group)
      }
    )) %>%
    unnest()
